...
function App() {
  ...
  const hasError = waitingTickets.error || calledTickets.error || servedTickets.error || queueStats.error;
    ...
    <div className="min-h-screen bg-gradient-to-br from-secondary-50 via-primary-50 to-secondary-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex items-center gap-2 text-xs text-secondary-600">
                <Clock className="w-3 h-3" />
                <span>{lastRefresh.toLocaleTimeString('fr-FR')}</span>
              </div>
              <ApiEnvironmentDisplay className="hidden md:block" />
              <ApiStatus />
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Colonne 1: File d'Attente (affichage seul) */}
              ...
              <div className="max-h-96 overflow-y-auto space-y-2">
                {waitingTickets.data?.length ? (
                  waitingTickets.data.map((ticket) => (
                    <div key={ticket.ticketNumber} className="p-3 bg-warning-50 border border-warning-200 rounded-lg">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-warning-800">Ticket #{ticket.ticketNumber}</p>
                            <p className="text-xs text-warning-600">{new Date(ticket.createdAt).toLocaleTimeString('fr-FR')}</p>
                          </div>
                        </div>
                        <div className="text-xs text-warning-600 bg-warning-100 px-2 py-1 rounded-full">
                          En attente
                        </div>
                      </div>
                    </div>
          ...
          {/* Colonne 2: Tickets Appelés (affichage seul) */}
              ...
              <div className="max-h-96 overflow-y-auto space-y-2">
                {calledTickets.data?.length ? (
                  calledTickets.data.map((ticket) => (
                    <div key={ticket.ticketNumber} className="p-3 bg-primary-50 border border-primary-200 rounded-lg">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-primary-800">Ticket #{ticket.ticketNumber}</p>
                            <p className="text-xs text-primary-600">{new Date(ticket.calledAt || ticket.createdAt).toLocaleTimeString('fr-FR')}</p>
                          </div>
                        </div>
          ...
          {/* Colonne 3: Tickets Servis (affichage seul) */}
              ...
              <div className="max-h-96 overflow-y-auto space-y-2">
                {servedTickets.data?.length ? (
                  servedTickets.data.map((ticket) => (
                    <div key={ticket.ticketNumber} className="p-3 bg-success-50 border border-success-200 rounded-lg">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-success-800">Ticket #{ticket.ticketNumber}</p>
                            <p className="text-xs text-success-600">{new Date(ticket.servedAt || ticket.createdAt).toLocaleTimeString('fr-FR')}</p>
                          </div>
                        </div>
